...
function BookAppointment() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (parseInt(bhk) < 1 || parseInt(bhk) > 6 || !Number.isInteger(Number(bhk))) {
      toast.error("BHK must be valid.", { autoClose: 2000 });
      return;
    }

    if (parseInt(floor) < 1 || parseInt(floor) > 10 || !Number.isInteger(Number(floor))) {
      toast.error("Floor number should be between 1 and 10", { autoClose: 2000 });
      return;
    }

    if (parseInt(sqft) < 500 || !Number.isInteger(Number(sqft))) {
      toast.error("Square Foot Area should be atleast 500", { autoClose: 2000 });
      return;
    }
